...
# ------------------------------- one vocabulary, hand-written and generated

@pytest.fixture(scope="module")
def both():
    """The published files and a generated part, served as one vocabulary."""
    from gemov import Config
    from gemov.server import from_sources
    config = os.path.normpath(os.path.join(ROOT, "..", "seas", "gen",
                                           "vocabulary"))
    if not os.path.isdir(config):
        pytest.skip("the SEAS 1.2 declaration is not next door")
    application = from_sources(
        directories=[SOURCES], namespace=SEAS_NS,
        config=Config.load(config), version="1.2", prefix="seas",
        provenance="hide")
    application.config["TESTING"] = True
    return application.test_client()


def test_a_module_has_the_versions_of_both_halves(both):
    source = both.application.config["SOURCE"]
    assert source.module_versions("GenericPropertyOntology") == ["1.0", "1.2"]
    assert source.module_versions("BuildingOntology") == ["1.0"]


def test_every_version_of_a_module_is_served(both):
    for path in ("/GenericPropertyOntology",           # the latest, generated
                 "/GenericPropertyOntology-1.2",
                 "/GenericPropertyOntology-1.0",       # the hand-written one
                 "/BuildingOntology"):                 # only hand-written
        assert both.get(path).status_code == 200, path


def test_a_term_does_not_say_which_half_minted_it(both):
    """The point: a reader has no business telling the halves apart."""
    generated = both.get("/TemperatureProperty").get_data(as_text=True)
    typed = both.get("/EnergyForm").get_data(as_text=True)
    for body in (generated, typed):
        assert body.count('href="pattern-') == 0
        assert "Generated by</h2>" not in body


def test_hiding_the_provenance_removes_the_reflexive_pages(both):
    assert both.get("/patterns").status_code == 404
    assert both.get("/").get_data(as_text=True).count('href="patterns') == 0


def test_showing_the_provenance_is_the_default():
    """gemov's own vocabularies keep the page a hand-written ontology cannot
    have; hiding it is a deployment's decision, not the library's."""
    from gemov import Config
    from gemov.server import from_sources
    application = from_sources(config=Config.load(EXAMPLE), version="1.0")
    application.config["TESTING"] = True
    client = application.test_client()
    assert client.get("/patterns").status_code == 200


def test_two_namespaces_are_two_vocabularies():
    from gemov.server.source import Combined, Files
    with pytest.raises(ValueError) as exc:
        Combined([Files([SOURCES], SEAS_NS),
                  Files([SOURCES], "https://example.org/")])
    assert "one namespace" in str(exc.value)
