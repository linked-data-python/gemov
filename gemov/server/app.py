...
from . import views
from .source import Combined, Files, Generated
...
def _facts():
    """What is known about the patterns — or nothing, deliberately.

    A vocabulary that is partly generated and partly hand-written is still
    one vocabulary, and `provenance="hide"` is how a deployment says its
    readers have no business telling the halves apart: no *Generated by*
    section on a module, no `/patterns`, no `/pattern-<name>`. The terms
    themselves are already indistinguishable — this is the last thing that
    would have marked them.
    """
    source, config = _source(), current_app.config["CONFIG"]
    if current_app.config["PROVENANCE"] == "hide":
        return []
...
def build_app(source, prefix="", config=None, mount="", brand=None,
              assets=None, order="kind", provenance="show"):
...
def from_sources(directories=(), namespace=None, config=None, version="1.0",
                 prefix="", mount="", brand=None, assets=None, order="kind",
                 provenance="show"):
    """One application over the files, the configuration, or **both**.

    A vocabulary that is partly hand-written and partly generated is still
    one vocabulary; this is how it is served as one.
    """
    parts = []
    if directories:
        if not namespace:
            raise ValueError("directories of files need a namespace")
        parts.append(Files(list(directories), namespace))
    if config is not None:
        parts.append(Generated(config, version))
    if not parts:
        raise ValueError("give directories of files, a configuration, or both")
    source = parts[0] if len(parts) == 1 else Combined(parts)
    return build_app(source, prefix, config, mount=mount, brand=brand,
                     assets=assets, order=order, provenance=provenance)
