...
def main(argv=None):
    ...
    serve.add_argument("config", nargs="?",
                       help="a gemov configuration — a file or a directory. "
                            "May be given TOGETHER with --files: a vocabulary "
                            "that is partly hand-written and partly generated "
                            "is still one vocabulary")
    serve.add_argument("--files", nargs="+",
                       help="directories of Module-x.y.ttl files")
    serve.add_argument("--namespace", help="required with --files")
    serve.add_argument("--version", default="1.0", dest="generated_version",
                       help="the version the GENERATED modules are published "
                            "at (default 1.0); generating at a version the "
                            "files already publish would put two graphs at "
                            "one versioned IRI")
    serve.add_argument("--provenance", default="show",
                       choices=("show", "hide"),
                       help="whether the pages say which terms a pattern "
                            "minted. `hide` serves a partly generated "
                            "vocabulary as one: no `Generated by`, no "
                            "/patterns")
    ...
    if args.command == "serve":
        try:
            from .server import from_sources
        except ImportError:
            raise SystemExit("the server is optional: pip install 'gemov[server]'")
        if not args.config and not args.files:
            raise SystemExit("give a configuration, or --files with "
                             "--namespace, or both")
        if args.files and not args.namespace:
            raise SystemExit("--files needs --namespace")
        application = from_sources(
            directories=args.files or (), namespace=args.namespace,
            config=Config.load(args.config) if args.config else None,
            version=args.generated_version, prefix=args.prefix,
            mount=args.mount, brand=_brand(args.brand), assets=args.assets,
            order=args.order, provenance=args.provenance)
