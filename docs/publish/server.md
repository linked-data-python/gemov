# Serving the namespace

```text
gemov serve vocabulary.yml                              # a generated vocabulary
gemov serve --files ontologies/ --namespace https://w3id.org/seas/ --prefix seas
```

A vocabulary published under **one namespace**, **modular** and **versioned**,
has to answer for every IRI under that namespace. This is that server. The
source is either a gemov configuration or a directory of `Module-x.y.ttl`
files, and **the contract below is the same either way**.

## The access contract

| IRI | denotes | answer |
|---|---|---|
| `…/` | the vocabulary | **200** — an index of the modules |
| `…/BuildingOntology` | the module | **200** — its current version, with `Content-Location` naming the versioned IRI |
| `…/BuildingOntology-0.9` | that version of the module | **200** |
| `…/BuildingOntology-0` | the highest version with that major | **302** to the full version |
| `…/EndNode` | a term | **200** — its definition, from the module version that claims it |
| `…/figure.png` | nothing in RDF | **200** — from `--assets`, if there is such a file |

Two rules cut across all of them.

**Content negotiation.** `Accept` decides among `text/turtle`,
`application/rdf+xml`, `application/ld+json`, `application/n-triples` and
`text/html`, and every answer carries `Vary: Accept` so that a shared cache
never serves HTML to a parser.

**An extension wins over `Accept`.** `…/BuildingOntology.ttl` and
`…/EndNode.html` name a representation directly — for a browser's address bar,
or a `curl` with no headers. SAREF publishes the same rule as `.conneg`
rewrites in Apache; here it is one function, and it is tested.

### Why the unversioned IRI is served and not redirected

SAREF redirects `core/` to `core/v3.1.1/`. This server does not, and the
difference is deliberate: the unversioned IRI **denotes the module itself**,
and dereferencing it gives the current version of that module. A redirect
would say instead that the module *is* its current version, which is false as
soon as there are two. `Content-Location` names the version actually served, so
a client that wants to pin one can follow it.

## Views

```text
/view?module=GenericPropertyOntology&term=Property&closure=1
/view?dimension=quantity=Temperature&dimension=kind=MechanicalQuantity
```

`module=`, `term=` (or `element=`) and `dimension=` may repeat. With
`closure=1` the graph is completed and the page lists what was pulled in and
why. On a generated vocabulary, `dimension=` is the
[profile](../model/profiles.md): the product is restricted, so what is not
asked for is never minted. On a source that is files there is nothing to
re-run, and asking for a dimension is a `400` that says so.

Views negotiate like everything else.

## Pattern pages

A generated vocabulary also serves `/patterns` and `/pattern-<name>` — see
[generated documentation](documentation.md).

## One vocabulary, several sources

A vocabulary is rarely all of one kind. SEAS is a cartesian product for its
99 quantities and hand-written prose for everything else; a regeneration
arrives one module at a time. The reader has no business telling the halves
apart, and does not have to:

```text
gemov serve gen/vocabulary --files ontologies/1.0 ontologies/0.9 \
            --namespace https://w3id.org/seas/ --prefix seas \
            --version 1.2 --provenance hide
```

The files and the configuration are served as **one** vocabulary. Merging is
per *(module, version)*: the same module at the same version in two sources is
one module version whose graph is the union, which is what lets a generated
family and a hand-written remainder share a module without either knowing
about the other. The namespace must be the same everywhere — two namespaces
are two vocabularies, and serving them as one would make every relative link
ambiguous.

**`--version` is not a detail.** It says at which version the *generated*
modules are published. Generating at `1.0` next to a published `1.0` would put
two different graphs at one versioned IRI, and the last source loaded would
quietly win.

**`--provenance hide`** removes the last thing that would mark the generated
half: no *Generated by* section on a module, no `/patterns`, no
`/pattern-<name>`. The terms themselves were already indistinguishable. The
default is `show`, because [the page a hand-written ontology cannot
have](documentation.md#the-page-a-hand-written-ontology-cannot-have) is worth
having; hiding it is a deployment's decision about its readers, not the
library's about its users.

## A context path

```text
gemov serve --files ontologies/ --namespace https://w3id.org/seas/ \
            --prefix seas --mount /seas
```

`--mount` is where the site is served under — the index at `/seas/`, a term at
`/seas/EndNode`. It is a **deployment** decision, not a property of the
vocabulary: the same files served at the root and behind a context path must
produce the same pages.

That works because almost every link a page carries is **relative**. From
`/seas/EndNode`, `href="Room"` resolves to `/seas/Room`; from `/EndNode` it
resolves to `/Room`. Only two links cannot be relative — the link back to the
index, and the redirect from a partial version — and both go through the
application's own routing, so they also pick up a WSGI `SCRIPT_NAME` if a
reverse proxy sets one instead.

## An identity

A published vocabulary belongs to a project, and the people who knew it by its
logo should still recognise it when the pages are regenerated by a different
tool years later.

```yaml
# brand.yml
name: SEAS
logo: static/logo.png          # relative, like every other link
home: http://the-smart-energy.com
note: Published by the ITEA 2 12004 SEAS project.
ink: "#3479aa"                 # the header, and the colour of links
accent: "#2b6187"
```

```text
gemov serve … --brand brand.yml --assets ./assets
gemov docs  … --brand brand.yml
```

`--assets` is a directory published under `<mount>/static/`, which is where
`logo.png` above is found. Both flags are optional and independent: without a
brand the pages are exactly as they were.

An asset is also served **directly under the namespace**, when nothing else
claims that name: SEAS descriptions point at
`https://w3id.org/seas/featureofinterest.png`, and that IRI is this server's to
answer — the 2016 site served those figures at exactly those IRIs, and a
documentation whose figures 404 is a documentation with holes in it. A module
always wins over a file of the same name.

Only those two colours are accepted, and on purpose. A header painted in a
palette that fights the logo looks worse than no logo at all; everything else
about the page is the stylesheet's business, and a theme system would be a
second way to write CSS.

## Notes for deployment

- an application is built **per source** (`build_app`, `from_files`,
  `from_config`), so several vocabularies can be served in one process, and
  there is no global state to trip over;
- `gemov serve` runs Flask's development server, which is right for a laptop
  and wrong for a deployment. In a container, build the application once at
  import and let gunicorn run it — `seas/deploy/` is a worked example, mount
  point and brand included;
- pages are cached, with `ETag` and `304`; `/cache` reports the statistics;
- for a vocabulary that never changes, `gemov docs` writes the same pages as
  files and any static server will do. The server exists for the views, which
  cannot be enumerated in advance.

## Installing

```text
pip install gemov[server]
```
